...
**Contact Email:** info@leadersperformance.nl
...
#### 3.2 Health Data
- **Step Count:** Daily step totals from Health Connect (Android) or HealthKit (iOS)
- **Calories Burned:** Calculated from step activity
- **Distance Walked:** Calculated from step count and stride length

#### 3.3 Location Data
- **GPS Coordinates:** Collected ONLY during active walking sessions
...
- Access health data when the app is closed (background access limited to sync only)
...
|---------|---------|-------------|
| Lovable Cloud (Supabase) | Backend infrastructure, authentication, database | Account data, step records |
...
| Precise location | Yes | No | GPS route tracking during sessions | Yes (can skip active sessions) |
| Health info (steps) | Yes | No | Step count verification | No |
...
- Contact Info (Email)
- Identifiers (User ID)
- Health & Fitness (Step Count)
- Location (Precise Location - during active sessions only)
